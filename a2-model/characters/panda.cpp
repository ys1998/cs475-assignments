...
namespace panda {
    ...
    HNode* create_character(std::vector<HNode*> &joints) {
        ...
        v.clear(); c.clear();
        make_hemisphere(v, c, 1.0, glm::vec4(1.0));
        Rigid_Node* torso = new Rigid_Node(waist, v, c, s);
        ...
        // neck
        v.clear(); c.clear();
        make_frustum(v, c, 2.51, 2.0, 0.0, 1.5, glm::vec4(0.0, 0.0, 0.0, 1.0), glm::vec4(0.0, 0.0, 0.0, 1.0));
        Rigid_Node* neck = new Rigid_Node(neck_joint_1, v, c);
        ...
        s = glm::scale(glm::mat4(1.0), glm::vec3(2.5, 2.5, 2.0));
        v.clear(); c.clear();
        make_sphere(v, c, 1.0, glm::vec4(1.0));
        Rigid_Node* head = new Rigid_Node(neck_joint_2, v, c, s);
        head->change_parameters(glm::vec3(0.0, 0.0, 1.0), glm::vec3(0.0));

        // ears
        v.clear(); c.clear();
        make_hemisphere(v, c, 1.0, glm::vec4(glm::vec3(0.0), 1.0));
        s = glm::scale(glm::mat4(1.0), glm::vec3(0.7, 0.2, 1.3));
        Rigid_Node* left_ear = new Rigid_Node(head, v, c, s);
        left_ear->change_parameters(glm::vec3(-1.5, 0.0, 1.0), glm::vec3(0.0));
        Rigid_Node* right_ear = new Rigid_Node(head, v, c, s);
        right_ear->change_parameters(glm::vec3(1.5, 0.0, 1.0), glm::vec3(0.0));

        // black eye patches
        v.clear(); c.clear();
        make_sphere(v, c, 1.0, glm::vec4(glm::vec3(0.0), 1.0));
        Rigid_Node* left_eye_patch = new Rigid_Node(head, v, c);
        left_eye_patch->change_parameters(glm::vec3(-0.7, 1.4, 0.4), glm::vec3(0.0));
        Rigid_Node* right_eye_patch = new Rigid_Node(head, v, c);
        right_eye_patch->change_parameters(glm::vec3(0.7, 1.4, 0.4), glm::vec3(0.0));

        // eyes
        v.clear(); c.clear();
        make_sphere(v, c, 0.5, glm::vec4(1.0));
        Rigid_Node* left_eye = new Rigid_Node(head, v, c);
        left_eye->change_parameters(glm::vec3(-0.8, 1.9, 0.6), glm::vec3(0.0));
        Rigid_Node* right_eye = new Rigid_Node(head, v, c);
        right_eye->change_parameters(glm::vec3(0.8, 1.9, 0.6), glm::vec3(0.0));

        v.clear(); c.clear();
        make_sphere(v, c, 0.2, glm::vec4(0.0, 0.5, 0.0, 1.0));
        Rigid_Node* left_eye1 = new Rigid_Node(head, v, c);
        left_eye1->change_parameters(glm::vec3(-0.8, 2.3, 0.6), glm::vec3(0.0));
        Rigid_Node* right_eye1 = new Rigid_Node(head, v, c);
        ...
        Rigid_Node* nose = new Rigid_Node(head, v, c, s);
        nose->change_parameters(glm::vec3(0.0, 1.7, 0.0), glm::vec3(0.0));

        // mouth
        v.clear(); c.clear();
        make_hemisphere(v, c, 1.0, glm::vec4(0.5, 0.0, 0.0, 1.0));
        s = glm::scale(glm::mat4(1.0), glm::vec3(1.0, 1.0, 0.3));
        s = glm::rotate(s, glm::radians(-90.0f), glm::vec3(1.0, 0.0, 0.0));
        Rigid_Node* mouth = new Rigid_Node(head, v, c, s);
        ...
        // left upper arm
        v.clear(); c.clear();
        make_frustum(v, c, 1.2, 1.4, 0.0, 2.5, glm::vec4(0.0, 0.0, 0.0, 1.0), glm::vec4(0.0, 0.0, 0.0, 1.0));
        Rigid_Node* left_upper = new Rigid_Node(left_shoulder, v, c);
        ...
        // left lower arm
        v.clear(); c.clear();
        make_frustum(v, c, 1.4, 1.0, 0.0, 4.0, glm::vec4(0.0, 0.0, 0.0, 1.0), glm::vec4(0.0, 0.0, 0.0, 1.0));
        Rigid_Node* left_lower = new Rigid_Node(left_elbow, v, c);
        ...
        Rigid_Node* left_hand = new Rigid_Node(left_wrist, v, c, s);
        ...
        v.clear(); c.clear();
        make_frustum(v, c, 1.2, 1.4, 0.0, 2.5, glm::vec4(0.0, 0.0, 0.0, 1.0), glm::vec4(0.0, 0.0, 0.0, 1.0));
        Rigid_Node* right_upper = new Rigid_Node(right_shoulder, v, c);
        ...
        v.clear(); c.clear();
        make_frustum(v, c, 1.4, 1.0, 0.0, 4.0, glm::vec4(0.0, 0.0, 0.0, 1.0), glm::vec4(0.0, 0.0, 0.0, 1.0));
        Rigid_Node* right_lower = new Rigid_Node(right_elbow, v, c);
        ...
        s = glm::scale(glm::mat4(1.0), glm::vec3(1.2, 1.0, 1.0));
        s = glm::rotate(s, glm::radians(180.0f), glm::vec3(0.0, 1.0, 0.0));
        Rigid_Node* right_hand = new Rigid_Node(right_wrist, v, c, s);
        ...
        // left thigh
        v.clear(); c.clear();
        make_frustum(v, c, 2.01, 1.5, 0.0, 2.25, glm::vec4(0.3137, 0.196, 0.0, 1.0), glm::vec4(0.5882, 0.3529, 0.0, 1.0));
        Rigid_Node* left_thigh = new Rigid_Node(left_hip, v, c);
        ...
        // left lower leg
        v.clear(); c.clear();
        make_frustum(v, c, 1.5, 1.0, 0.0, 2.0, glm::vec4(0.0, 0.0, 0.0, 1.0), glm::vec4(0.0, 0.0, 0.0, 1.0));
        s = glm::rotate(glm::mat4(1.0), glm::radians(-90.0f), glm::vec3(1.0, 0.0, 0.0));
        Rigid_Node* left_lower_leg = new Rigid_Node(left_knee, v, c, s);
        ...
        // left foot
        v.clear(); c.clear();
        make_hemisphere(v, c, 1.0, glm::vec4(0.3137, 0.196, 0.0, 1.0));
        s = glm::scale(glm::mat4(1.0), glm::vec3(1.0, 1.25, 1.7));
        s = glm::rotate(s, glm::radians(90.0f), glm::vec3(1.0, 0.0, 0.0));
        Rigid_Node* left_foot = new Rigid_Node(left_ankle, v, c, s);
        ...
        v.clear(); c.clear();
        make_frustum(v, c, 2.01, 1.5, 0.0, 2.25, glm::vec4(0.3137, 0.196, 0.0, 1.0), glm::vec4(0.5882, 0.3529, 0.0, 1.0));
        Rigid_Node* right_thigh = new Rigid_Node(right_hip, v, c);
        ...
        make_frustum(v, c, 1.5, 1.0, 0.0, 2.0, glm::vec4(0.0, 0.0, 0.0, 1.0), glm::vec4(0.0, 0.0, 0.0, 1.0));
        s = glm::rotate(glm::mat4(1.0), glm::radians(-90.0f), glm::vec3(1.0, 0.0, 0.0));
        Rigid_Node* right_lower_leg = new Rigid_Node(right_knee, v, c, s);
        ...
        s = glm::scale(glm::mat4(1.0), glm::vec3(1.0, 1.25, 1.7));
        s = glm::rotate(s, glm::radians(90.0f), glm::vec3(1.0, 0.0, 0.0));
        Rigid_Node* right_foot = new Rigid_Node(right_ankle, v, c, s);
