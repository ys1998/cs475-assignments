...
extern GLuint vPosition, vColor, vNormal, texCoord, uModelViewMatrix, uNormalMatrix, ViewMatrix, applyTexture;
...
void HNode::init_buffers() {
	...
	glBufferData(GL_ARRAY_BUFFER, t*(_vertices.size() + _colors.size()) + sizeof(glm::vec3)*_normals.size() + sizeof(glm::vec2)*_texspace.size(), NULL, GL_STATIC_DRAW);
	...
	glBufferSubData(GL_ARRAY_BUFFER, t*_vertices.size() + t*_colors.size(), sizeof(glm::vec3)*_normals.size(), &_normals[0]);
	glBufferSubData(GL_ARRAY_BUFFER, t*_vertices.size() + t*_colors.size() + sizeof(glm::vec3)*_normals.size(), sizeof(glm::vec2)*_texspace.size(), &_texspace[0]);
	...
	glVertexAttribPointer(vNormal, 3, GL_FLOAT, GL_FALSE, 0, BUFFER_OFFSET(t*_vertices.size()+t*_colors.size()));
	glEnableVertexAttribArray(texCoord);
	glVertexAttribPointer(texCoord, 2, GL_FLOAT, GL_FALSE, 0, BUFFER_OFFSET(t*_vertices.size()+t*_colors.size()+sizeof(glm::vec3)*_normals.size()));
...
std::vector<float> HNode::get_parameters(void){
	std::vector<float> res(0);
	res.push_back(_parameters[0].x); res.push_back(_parameters[0].y); res.push_back(_parameters[0].z);
	res.push_back(_parameters[1].x); res.push_back(_parameters[1].y); res.push_back(_parameters[1].z);
	return res;
}
...
void HNode::render_node(){
	...
	glm::mat3 normalMatrix = glm::transpose(glm::inverse(glm::mat3(MatrixStack.top())));
	...
	glUniform1i(applyTexture, isTextured ? 1 : 0);
...
void HNode::set_texture( const char * filename, int width, int height )
{
	isTextured = true;
	_tex = LoadTexture(filename, width, height);
}

void HNode::remove_texture()
{
	isTextured = false;
	FreeTexture(_tex);
}

void HNode::set_texspace(std::vector<glm::vec2> &texspace)
{
	_texspace = texspace;
}
...
Hinge_Joint::Hinge_Joint(
    ...
    make_sphere(_vertices, _colors, _normals, 1.0, color);
}
