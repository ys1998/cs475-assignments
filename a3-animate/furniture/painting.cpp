#include "furniture.hpp"
#include "texture.hpp"
#include "hierarchy_node.hpp"
#include "parts.hpp"
#include <vector>
#include <iostream>
#include <glm/gtx/string_cast.hpp>

namespace painting {
    // Function to create the character
    // returns pointer to root node, and adds all joints to vector
    HNode* create_structure(HNode *parent) {
        // list of vertices, colors
        std::vector<glm::vec4> v, c;
        std::vector<glm::vec3> n;
        std::vector<glm::vec2> t;

        // define box geometry
        make_texquad(v, c, n,
            glm::vec4(0.0, 0.0, 20.0, 1.0),
            glm::vec4(0.0, 0.0, 70.0, 1.0),
            glm::vec4(0.0, 50.0, 70.0, 1.0),
            glm::vec4(0.0, 50.0, 20.0, 1.0),
            glm::vec4(), glm::vec4(), glm::vec4(), glm::vec4(), t
        );

        Rigid_Node* root = new Rigid_Node(parent, v, c, n);
        root->change_parameters(glm::vec3(99.0, 0.0, 20.0), glm::vec3(90.0,0.0,0.0));
        root->set_name("Painting");
        root->set_texspace(t);
        root->set_texture("images/panzer.bmp", 1024, 739);
        
        return root;
    }
}